...
use chainbound_artemis::{
    Action, BlockBuilder, EchoExecutor, Event, FiberCollector, SendBundleArgs, StreamType,
};
use ethers::{
    prelude::rand,
...
impl Strategy<Event, Action> for MevStrategy<LocalWallet> {
    ...
    async fn process_event(&mut self, event: Event) -> Vec<Action> {
        ...
        let fake_backrun = TransactionRequest::new()
            .to(account)
            .from(account)
            .value(42)
        ...
        let bundle = Action::SendBundle(
            SendBundleArgs::with_txs(vec![fake_backrun])
